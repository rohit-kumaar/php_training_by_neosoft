...
if(isset($_POST['regis'])){
    ...
    if(empty($email) || empty($pass) || empty($capp) || empty($name || empty($age) || empty($city) || empty($gender) || empty($tmp))){
        $errMsg="Please fill blank fields";
    }
    else {
        if(substr(md5($capp),0,5)==$_POST['hid'])
        {
    ...
    <main class="container">
        ...
        <section>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label> <?= $cap;?> </label>
                    <input type="text" name="capp" class="form-control"/>
                    <input type="hidden" name="hid" value="<?= substr(md5($sum),0,5);?>"/>
                </div>
                <div class="form-group">
